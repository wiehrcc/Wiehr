...
(function () {
    ...
    /* One place that knows what is animating, so one call can stop all of it.

       Every continuous loop on the site used to publish itself by assigning
       window.pauseWebGL. That is a single slot: floatingdust.js (3rd script)
       set it, logolookslikevenom.js (6th) overwrote it outright, and
       networkgraph.js (8th) chained onto whatever happened to be there. Whose
       loop actually paused depended on document order, and the dust's never
       did.

       A registry has no order to get wrong. Modules add themselves whenever
       they load; pauseAll/resumeAll fan out to all of them. pauseWebGL and
       resumeWebGL stay as aliases because existing callers use those names.

       This matters because of what sits on top: every overlay on the site -
       the menu and every .staringcontest modal - is a full-viewport
       backdrop-filter, and a backdrop-filter recomputes whenever what is
       behind it changes. Anything still drawing under one is paying for a
       full-screen blur per frame to render pixels nobody can see. */
    var loops = [];
    window.WiehrLoops = {
        add: function (pause, resume) {
            loops.push({ pause: pause, resume: resume, paused: false });
        },
        pauseAll: function () {
            loops.forEach(function (l) {
                if (!l.paused) { l.paused = true; try { l.pause(); } catch (e) {} }
            });
        },
        resumeAll: function () {
            loops.forEach(function (l) {
                if (l.paused) { l.paused = false; try { l.resume(); } catch (e) {} }
            });
        }
    };
    window.pauseWebGL = function () { window.WiehrLoops.pauseAll(); };
    window.resumeWebGL = function () { window.WiehrLoops.resumeAll(); };
