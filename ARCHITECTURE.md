...
4. **Unlocked** — the same two buttons change what they say:
   `LICENSE FILES` | `{CUSTOM LABEL}`, and the contact row disappears.
...
build the `?key=` link behind LICENSE FILES, and that link is what makes
...
### Overlays stop what is under them

An overlay here — the menu, and every `.staringcontest` modal — is a
full-viewport `backdrop-filter`, and a backdrop-filter recomputes whenever what
is behind it changes. So any loop still drawing under one pays for a
full-screen blur every frame to produce pixels the blur then hides. That is
what made opening the menu, or an image on /storage, cost what it did.

`WiehrOverlay.open()` now pauses every registered loop and `close()` resumes
them, so it happens once for every overlay on the site including any added
later. The menu used to do this by hand; no modal did it at all.

**`WiehrLoops` (in `howfastareyou.js`) is why that is reliable.** Every
continuous loop used to publish itself by assigning `window.pauseWebGL` — a
single slot. `floatingdust.js` (3rd script) set it, `logolookslikevenom.js`
(6th) overwrote it outright, and `networkgraph.js` (8th) chained onto whatever
happened to be there. Which loop actually paused depended on document order,
and the dust's never did. A registry has no order to get wrong; modules add
themselves as they load and `pauseAll`/`resumeAll` fan out.

The menu also stacked **two** full-area backdrop-filters: the veil blurs the
whole viewport, and the pane on top of it blurred that already-blurred image
again through an SVG `feTurbulence` + `feDisplacementMap` over 640×585 — for a
difference you cannot see. The pane now takes a plain fill over the veil and
keeps its hairline and specular edges.

### The liquid glass is a luxury, spent in one place

`url()` **is** supported in `backdrop-filter` in Chromium — confirmed with
`CSS.supports` rather than assumed — so `#wiehr-liquid-glass` genuinely runs:
feTurbulence into a Gaussian blur into a feDisplacementMap, as a *backdrop*
filter, meaning it re-runs whenever anything behind it changes.

It was applied to `.acrossthetop` — the header, full-bleed, on **every page**,
repainting on every scroll — and to every cell of the storage grid, and to
`.flipthrough`. Across 13 routes that was **21 displacement-map backdrops**.

It now survives on one thing: the single large content pane of a page, on the
top tier only. **21 → 5 on `high`, 0 on `mid` and `low`.** The header and the
grid cells take a plain blur, which at a hairline behind type or at cell size
is indistinguishable from the warp anyway. One big pane is the only place the
optical distortion was ever legible, which is the whole argument for it.

The breadcrumb also carried its own fill and shadow in dark theme, a second
glass panel floating inside the header's. Removed — the header is one bar.

Backdrop images were checked and cleared: 56–75KB each, one per page.

### What actually made opening the menu cost

Three things, and the largest was not the menu at all:

1. **`triggerGlitch()` filtered the entire `<body>` through an SVG displacement
   map.** `body.themeglitch` ran `url(#wiehr-noise-filter)` — feTurbulence into
   feDisplacementMap — over every pixel on screen for 350ms, and the menu
   calls it on open. So the most expensive filter a browser has was applied to
   the whole page, including the menu's own veil and its backdrop-filter, at
   the exact moment the menu was animating in. That is the stutter, and the
   reason the cursor fell behind. The keyframes now use `brightness` and
   `contrast`, which are cheap GPU filters and carry the same flash; the grain
   is gone, the glitch still reads as a glitch. Off entirely below the top
   tier.

2. **The hover preview re-rendered a full-screen filtered image per row
   crossed.** `.wherewewouldgo` is larger than the viewport and carries
   `filter: invert() contrast() blur()`; swapping its background-image
   invalidates that filter, so it re-renders whole. Bound straight to
   `pointerenter`, dragging down the menu fired one of those per row. A 90ms
   settle means only the row you stop on renders — measured: **11 rows crossed
   went from 11 full-screen renders to 1**.

3. **The row-hover turbulence.** `wiehrnoiseanim` is fine on a link; a menu row
   is 541×60, and crossing the menu starts several overlapping. Off below the
   top tier, like the body glitch.

The veil blur is now a token and tiers with the device: **16 / 10 / 6px**. It
is the one number in the menu worth tiering, since a blur kernel grows with its
radius and this one covers the viewport.

### The logo's hot loop

Three per-frame allocation sites and a pile of redundant trigonometry:

- `getProjected3D()` recomputed `cos`/`sin` of the same two rotation angles
  **per particle**, though they change once per frame — a quarter of a million
  redundant trig calls a second at 1000 particles — and returned a fresh
  `{x, y, scale}` read twice and discarded. `projectInto()` takes the frame's
  trig and writes straight into the position buffer. Verified bit-identical to
  the old path across 20,000 random inputs.
- `update(..., [], ...)` allocated an empty array per particle per frame.
- The network mirror rebuilt its array and one object per node every frame,
  though its consumers read it fresh and hold no references.

`FRAME_INTERVAL` stays pinned at 60, not the device tier: the particle
integration is per-frame, so capping the rate runs the animation in slow
motion. The tier's saving on this file comes off density instead.
